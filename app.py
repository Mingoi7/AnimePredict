...
file_path = 'model/anime-weights.h5'
...
with open('model/anime-encoder.pkl', 'rb') as file:
    anime_encoder = pickle.load(file)

with open('model/user-encoder.pkl', 'rb') as file:
...
# Random Selection
def random_select_animes(name, n=10, return_dist=False, neg=False):
    try:
        anime_row = df_anime[df_anime['Name'] == name].iloc[0]
        index = anime_row['anime_id']
        encoded_index = anime_encoder.transform([index])[0]
...
def get_all_animes (name , n=10, return_dist=False, neg=False):
    try:
        anime_row = df_anime[df_anime['Name'] == name].iloc[0]
        index = anime_row['anime_id']
        encoded_index = anime_encoder.transform([index])[0]
        ...
        for close in closest:
            ...
            SimilarityArr.append({"anime_image_url": anime_image_url,"Name": anime_name, "Genres": genre,
                                  "Synopsis":Synopsis,"English Name": english_name,"Native name": other_name,"Score": score,"Type": Type,
                                  "Aired": aired, "Premiered": premiered, "Episodes": episodes, "Status": status, "Studios": studios,
                                  "Source": source, "Rating": rating, "Rank": rank, "Favorites": favorites,"Duration": duration,
                                  "Episode Duration": episode_duration,"anime_id":anime_id})
...
@app.route('/anime')
def anime():
    anime_list = df_anime['Name'].values.tolist()
    anime_name = random.choice(anime_list)
    message2 = "For Your recommendation Anime List : " + str(anime_name)
    all_animes = get_all_animes(anime_name,1000, return_dist=False, neg=False)
    if all_animes is None or all_animes.empty:
        message2 = "Ops! Anime ! Try Again!"
        return render_template('anime.html',message=message2, animes=None)

    return render_template('anime.html',message=message2,  animes=all_animes)
...
def recommend():
    ...
    if recommendation_type == "user_based":
        ...
        message1 = "Top "+str(num_recommendations) +" of Your ID " + str(user_id) + " recommendation Anime List that never seem before :"
        ...
        return render_template('recommendations.html', message=message1, animes=recommended_animes, recommendation_type=recommendation_type)

    elif recommendation_type == "item_based":
        anime_name = request.form['anime_name']
        message2 = "Top "+str(num_recommendations) +" of Your recommendation Anime List : " + str(anime_name)
        ...
        return render_template('recommendations.html', message=message2, animes=recommended_animes, recommendation_type=recommendation_type)

    elif recommendation_type == "random_select":
        anime_list = df_anime['Name'].values.tolist()
        anime_name = random.choice(anime_list)
        message2 = "Top "+str(num_recommendations) + " Lucky recommendation Anime List "

        if not anime_name:
            return render_template('index.html', error_message="Please enter Anime name.", recommendation_type=recommendation_type)
        
        recommended_animes = random_select_animes(anime_name, n=num_recommendations, return_dist=False, neg=False)

        if recommended_animes is None or recommended_animes.empty:
            message2 = "Ops! Try again!"
            return render_template('recommendations.html', message=message2, animes=None, recommendation_type=recommendation_type)
